...
class Detour {
  ...
  constructor (options = {}) {
    ...
    // backwards compatibility; should not support `caseSensitive` in next version
    this._options.sensitive = options.sensitive || options.caseSensitive;
  }
  ...
  route (path, resource, routeOptions) {
    validatePath(path);
    validateResource(resource);
    const route = new Route(path, resource, this._routeOptions(routeOptions));
    this._routes.push(route);
    return this;
  }

  collection (path, pairObj, routeOptions) {
    if (pairObj.collection == null) {
      throw new Error(`Detour.collection() requires an object with a \`collection\` property.  Path was: ${path}`);
    }

    const opts = this._routeOptions(routeOptions);

    if (pairObj.member) {
      this.route(path, pairObj.member, opts);
    }

    this.route(parentPath(path), pairObj.collection, opts);
    return this;
  }

  _routeOptions (overrides) {
    return Object.assign({}, this._options, overrides);
  }
}
