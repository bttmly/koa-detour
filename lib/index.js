...
const _complain = require("complain");
...
class Detour {
  ...
  middleware () {
    return (ctx, next) => {
      return pTry(() => this._dispatch(ctx, next))
  ...
  handleError (fn) {
    complain("`handleError()` is deprecated; call `.catch()` on the result of `middleware()` instead");
    this._handleErr = fn; return this;
  }

  // to special-handle resolutions
  handleSuccess (fn) {
    complain("`handleSuccess` is deprecated, call `.then()` on the reuslt of `middleware()` instead");
    this._handleOk = fn; return this;
  }
...
function complain (...args) {
  if (isTesting) return;
  return _complain(...args);
}
